...
grouped_df = male_una_data.groupby('age_bracket').sum().reset_index()
...
# Calculate the percentage of Male UNA relative to the total for each age bracket
...
fig = px.bar(
    ...
    x='age_bracket', 
    y='Percentage',
    title='Percentage of Male UNA Voters in 2024 by Age Bracket',
    ...
    hover_data={
        ...
        'Count': ':.0f'  # Include the count of Male UNA votes in hover data
    },
    color_discrete_sequence=['light blue'],  # Use distinct colors for each age group
)
