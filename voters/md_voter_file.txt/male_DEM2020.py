...
# Filter the data for Male Democrats (DEM)
male_dem_df = df[(df['Gender'] == 'Male') & (df['Party'] == 'DEM')]

# Group the data by age bracket and sum the counts for voted_2020
age_bracket_voted_2020 = male_dem_df.groupby('age_bracket')['voted_2020'].sum().reset_index()
...
# Merge total counts with the Male DEM data
merged_data = pd.merge(age_bracket_voted_2020, total_counts, on='age_bracket')

# Calculate the percentage of Male DEM voters who voted in 2020 relative to the total for each age bracket
...
fig = px.bar(
    ...
    x='age_bracket',
    y='Percentage',
    title='Male DEM Voter Participation in 2020 by Age Group',
