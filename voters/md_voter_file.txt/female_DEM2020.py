...
# Filter the data for Female Democrats (DEM)
female_dem_df = df[(df['Gender'] == 'Female') & (df['Party'] == 'DEM')]

# Group the data by age bracket and sum the counts for voted_2020
age_bracket_voted_2020 = female_dem_df.groupby('age_bracket')['voted_2020'].sum().reset_index()
...
# Merge total counts with the Female DEM data
merged_data = pd.merge(age_bracket_voted_2020, total_counts, on='age_bracket')

# Calculate the percentage of Female DEM voters who voted in 2020 relative to the total for each age bracket
...
fig = px.bar(
    ...
    x='age_bracket',
    y='Percentage',
    title='Female DEM Voter Participation in 2020 by Age Group',
    ...
    color_discrete_sequence=['blue'] * len(merged_data)  # Custom color palette
)
