...
age_bracket_votes = female_dem_df.groupby('age_bracket')[['voted_2024', 'voted_2020']].sum().reset_index()

# Calculate the percentage of votes for 2024 and 2020
total_counts = female_dem_df.groupby('age_bracket')['Count'].sum().reset_index()
...
fig = px.bar(
    ...
    title='Percentage Difference Between Voted 2024 and Voted 2020 for Female Democrats by Age Group',
...
fig.update_layout(
    ...
    xaxis_tickangle=-45,
    yaxis=dict(range=[-5, 40]),  # Set y-axis range with minimum at -5 and maximum at 40
)
